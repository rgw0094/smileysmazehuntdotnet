...
namespace Smiley.Lib.Menu
{
        ...
        public override void Draw()
        {
            ...
            foreach (KeyValuePair<SaveSlot, SaveBox> kvp in _saveBoxes)
            {
                ...
                if (_selectedSlot == kvp.Key && _deletePromptActive)
                {
                    ...
                    SMH.Graphics.DrawString(SmileyFont.Curlz, "Delete this file?", box.X + 75f, box.Y + 5f, TextAlignment.Left);

                    SMH.Graphics.DrawString(SmileyFont.Description, "Yes", box.X + 125f, box.Y + 65f, TextAlignment.Center, _mouseOverYes ? Color.Red : Color.Black);
                    SMH.Graphics.DrawString(SmileyFont.Description, "No", box.X + 205f, box.Y + 65f, TextAlignment.Center, _mouseOverNo ? Color.Red : Color.Black);
                }
                else
                {
                    ...
                    //File Name
                    SMH.Graphics.DrawString(SmileyFont.Inventory, SMH.SaveManager.Saves[_selectedSlot].IsEmpty ? "- Empty -" : "Save File " + i, box.X + 70f, box.Y + 5f, TextAlignment.Left, color, 2f);

                    //Time Played
                    SMH.Graphics.DrawString(SmileyFont.Description, "Time Played: ", box.X + 70f, box.Y + 50f, TextAlignment.Left, color, 0.8f);
                    SMH.Graphics.DrawString(SmileyFont.Description, SMH.SaveManager.Saves[_selectedSlot].IsEmpty ? "0:00:00" :
                        SMH.SaveManager.Saves[_selectedSlot].TimePlayed.ToString(), box.X + 250f, box.Y + 50f, TextAlignment.Right, color);

                    //Completion percentage
                    SMH.Graphics.DrawString(SmileyFont.Description, "Complete: ", box.X + 70f, box.Y + 75f, TextAlignment.Left, color);
                    SMH.Graphics.DrawString(SmileyFont.Description, SMH.SaveManager.Saves[_selectedSlot].IsEmpty ? "0%" :
                        SMH.SaveManager.Saves[_selectedSlot].CalculateCompletionPercentage().ToString(), box.X + 250f, box.Y + 75f, TextAlignment.Right, color);
                }
            ...
            else if (State == MenuState.ExitingScreen)
            {
                ...
                if (_windowY <= -512f)
                {
                    ...
                    switch (_clickedButton)
                    {
                        ...
                        //MainMenu.OpenLoadScreen(_selectedSlot, true);
                            break;
                    }
                }
            }
    ...
    public class DifficultyPrompt
    {
        ...
        public void Draw()
        {
            ...
            SMH.Graphics.DrawString(SmileyFont.Inventory, "Difficulty", _x + 162.5f, _y + 15f, TextAlignment.Center);
            SMH.Graphics.DrawString(SmileyFont.Inventory, _currentSelection.GetDescription(), _x + 162.5f, _y + 65f, TextAlignment.Center, Color.Black, 0.75f);
